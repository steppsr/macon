...
# 1st paramter is collection_id
# Make sure the first parameter isn't blank
if [ -z "$1" ]; then
...
# Encoded_id is the value we know as "NFT ID"
nft_list=$(curl -s https://api.mintgarden.io/collections/$collection_id/nfts/ids | jq -r '.[] | .encoded_id')

# Start with an empty string and we will loop through ever NFT and add it's JSON to out variable.
...
# Get the collection from the last NFT. Also replace any spaces with an underscore
...
# Be sure to replace any spaces in the collection name with an underscore
